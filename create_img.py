...
def get_im_frame(rank):
    if rank == 1:
        ...
        frame_tmp = 'color.png'
    im_frame = Image.open(f'{path}/img/frame/{frame_tmp}').convert("RGBA")  # 头像框
    return im_frame
...
def _generate_info_pic_internal(data):
    ...
    w, h = font_resize.getsize("好友数量:" + friend_num)
    draw.text((550 - w, 350), "好友数量:" + friend_num, font_black, font_resize)
    ...
    arena_time_text = _TraditionalToSimplified(time.strftime(
        "%Y/%m/%d %H:%M", time.localtime(data["user_info"]["arena_time"])))
    ...
    grand_arena_time_text = _TraditionalToSimplified(time.strftime(
        "%Y/%m/%d %H:%M", time.localtime(data["user_info"]["grand_arena_time"])))
    ...
    w, h = font.getsize(
        viewer_id_arr[0] + "  " + viewer_id_arr[1] + "  " + viewer_id_arr[2] + "  " + viewer_id_arr[3] + "  " +
        viewer_id_arr[4])
    draw.text((138 + (460 - 138) / 2 - w / 2, 1058),
              viewer_id_arr[0] + "  " + viewer_id_arr[1] + "  " + viewer_id_arr[2] + "  " + viewer_id_arr[3] + "  " +
              viewer_id_arr[4], (255, 255, 255, 255), font)
...
def _generate_support_pic_internal(data):
    ...
    支援界面图片合成
    '''
    im = Image.open(f'{path}/img/support.png').convert("RGBA")  # 支援图片模板
